...
func shoot(position: Vector2, dir: Vector2, speed: float, bullet_damage_value: int, bullet_knockback_value: int):
	spread_shoot(position, dir, speed, bullet_damage_value, bullet_knockback_value)


func spread_shoot(position: Vector2, dir: Vector2, speed: float, bullet_damage_value: int, bullet_knockback_value: int):
	var directions = []
	var angle := deg2rad(spread_angle)
	var start_angle := amount/2.0 * angle
	for i in range(amount):
		shoot_single(position, dir.rotated(start_angle - angle * i), speed, bullet_damage_value, bullet_knockback_value)
